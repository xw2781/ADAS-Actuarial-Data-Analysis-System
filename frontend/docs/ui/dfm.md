...
- DFM method JSON identity/save naming uses `projects/<project>/methods/DFM@<ReservingClass>@<Name>@<DevelopmentLength>@<OriginLength>.json` from Details inputs; the project is represented by the containing folder, not repeated in the JSON filename. The `ReservingClass` filename component replaces Windows-invalid filename characters with `^`.
