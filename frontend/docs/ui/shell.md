...
- Uses Electron host bridge for desktop-only ArcBot Codex CLI status, install, login, persisted chat sessions, streamed activity/output events, and `codex exec` requests; for Edit Mode, the host writes the active page JSON into a local ArcBot session folder using the canonical DFM method shape grouped by GUI tab, including DFM analysis snapshots for rounded input-data triangle values, ratio-triangle values, average-formula values, and the input triangle CSV path when the active tab is DFM. ArcBot edits only that temporary copy, then the host validates the target path against the configured Server Connection root, backs up the original JSON under the method `history` folder or the active page snapshot when direct read is denied, applies the edited grouped temp copy with the same row-compact formatter as normal saves, and can revert the latest ArcBot edit from that backup.
...
- ArcBot chat includes a session picker/new-session action, an in-panel Chat History page with open/archive/delete actions, a collapsible context panel with active-tab and estimated context usage details, an in-chat elapsed-time work summary card that stays expanded while a request is running and collapses when complete, a resizable chat panel, auto-growing frameless composer input, and a title-bar debug drawer with copyable raw stdout/stderr and host event logs.
